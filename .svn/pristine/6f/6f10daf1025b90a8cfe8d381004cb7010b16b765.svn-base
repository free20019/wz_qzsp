...
var regJgyjs1 = /^((1[5-9])|([2-9][0-9])|(\d{3,4}))(\.([0-9]{1,2}))?$/;
/*工商注册号*/
var regGszch = /^\d{15}$/;
/*服务机构名称*/
//var regFwjgmc = /^(([\u4e00-\u9fa5]{2,50})|([a-zA-Z\.\s\,]{2,50}))$/i;
var regFwjgmc= /^[\u4E00-\u9FA5A-Za-z0-9]+$/;
/*统一社会信用*/
...
/*日期验证*/
var regDate = /^(1[5-9][0-9]{2}|2\d{3})-(0[1-9]|1[0-2])-(0[1-9]|(1|2)\d|3[0-1])$/;
/*车辆识别号*/
var regClsbh = /^[A-Z,0-9]{17}$/;
/*出生日期*/
var regCsrq = /^\d{4}(-\d{2}){2}/;
/*车辆长宽高*/
var regCkg = /^[0-9]{1,5}$/;
/* 保险额度*/
var regBxed = /^[0-9]{1,8}$/;
...
var footerHtml = function() {
	return '<div class="foolerPage"><p>建议使用IE11.0以上版本、最新版火狐或谷歌浏览器，1024*768分辨率浏览</p><p>建设单位：温州市道路运输管理局&nbsp;|&nbsp;技术支持：杭州通悟科技有限公司</p><p>&nbsp;备案号：浙ICP备 号&nbsp;</p></div>';
};
$(function () {
...
var isIE6 = function() {
	...
	if (json.browser=="Microsoft Internet Explorer" && json.trim_Version=="MSIE6.0") { return true; }
	return false;
};
var isIE7 = function() {
	var json = obtainBrowser();
	if(json.browser=="Microsoft Internet Explorer" && json.trim_Version=="MSIE7.0") { return true; }
	return false;
};
var isIE8 = function() {
	var json = obtainBrowser();
	if(json.browser=="Microsoft Internet Explorer" && json.trim_Version=="MSIE8.0") { return true; }
	return false;
};
var isIE9 = function() {
	var json = obtainBrowser();
	if (json.browser == "Microsoft Internet Explorer" && json.trim_Version == "MSIE9.0") { return true; }
	return false;
};
/**
 * 信息提醒模板
...
var printHtml = function(strPrint) {
	...
	oWin.focus();
	setTimeout(function () { oWin.print(); }, 50);
	oWin.document.close();
};

function  bindImageChange (){
$(window.top.document.body).find('[id^=fileupload]').bind('change',function(event){
...
$(document).ready(function() {
	if (isIE7() || isIE8()) {
		...
		btnInfo.css({'position': 'relative', 'background': 'none'})
			.append("<div class='round_left'></div>")
			.append("<div class='round_right'></div>")
			.append(function() {
				var btnInfoText = $(this).text();
				return "<div class='round_bot' style='line-height:34px;'>"+btnInfoText+"</div>";
			});
		var btndanger=$(".btn-danger");
		btndanger.css({'position':'relative','background':'none'})
			.append("<div class='left_danger'></div>")
			.append("<div class='right_danger'></div>")
			.append(function(){
				var btndangerText=$(this).text();
				return "<div class='bot_danger' style='line-height:34px;'>"+btndangerText+"</div>"
			});
	}
});
//if (!jQuery.cookie){
