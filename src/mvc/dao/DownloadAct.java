...
//	}
	...
	public static List<Map<String, Object>> parseJSON2List(String msg){
		...
		String b = j.getString("datas");
        JSONArray jsonArr = JSONArray.fromObject(b);
        List<Map<String, Object>> list = new ArrayList<Map<String,Object>>();
        Iterator<JSONObject> it = jsonArr.iterator();
        while(it.hasNext()){
            JSONObject json2 = it.next();
...
//			System.out.println(list.get(i).get("COMP_NAME"));
//		}
        return list;
    }
    ...
    public static String download2(HttpServletRequest request,
                                  HttpServletResponse response,
                                  String[] a,String[] b,String[] c,String[] d,String gzb,List<Map<String,Object>> list) throws IOException{
        String fileName=gzb;//excle文件名
        //填充projects数据
        String columnNames[]=a;//列名
        String keys[]    =     b;//map中的key
        String columnNames2[]=c;//列名
        String keys2[]    =     d;//map中的key
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        try {
            ExcelUtil.createWorkBook2(list,keys,columnNames,keys2,columnNames2,gzb).write(os);
        } catch (IOException e) {
            e.printStackTrace();
        }
        byte[] content = os.toByteArray();
    ...
    public static String download3(HttpServletRequest request,
                                   HttpServletResponse response,
                                   String[] a,String[] b,String gzb,List<Map<String,Object>> list) throws IOException{
        String fileName=gzb;//excle文件名
        //填充projects数据
