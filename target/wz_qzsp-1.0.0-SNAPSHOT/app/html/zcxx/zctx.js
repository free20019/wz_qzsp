...
$(function () {
    ...
    $('#yzm').on('focus', function () {
        hint_info('yzm', '请输入验证码，四位数字 例：1111', 1);
    }).on('blur', yzmReg);
...
//    } else {
//        successInputBorderStyle(tyshxydm);
//    }
    //证件号码
    if (zzjgdmz.val() == "" | !regzzjgdm.test(zzjgdmz.val())) {
    ...
    if (qylxdh.val() == "" || !regPhone.test(qylxdh.val())) {
        ...
        successInputBorderStyle(qylxdh);
    }
    //经办人姓名
...
function qy_tj(){
    ...
    //证件号码
    if($("#zzjgdmz").val()==""){
        Pop_up_box("组织机构代码证号码不能为空","2");
        return false;
    }else if(!regzzjgdm.test($("#zzjgdmz").val())) {
        Pop_up_box("组织机构代码证号码格式不正确","2");
        return false;
    }
    //法人代表姓名
    if($("#frdbxm").val()==""){
...
//    }
    ...
    if($('#fileupload_zzjgdmzzp').val()==''){
		...
		if(a!='jpg'&&a!='jpeg'&&a!='JPG'&&a!='JPEG'&&a!='png'&&a!='PNG'){
			Pop_up_box("组织机构代码证或企业营业执照（五证合一）图片只能上传.jpg或者.JPEG、png格式的图片！","2");
			return false;
		}
	}
    if($('#fileupload_jbrsfzm').val()==''){
		...
		if(a!='jpg'&&a!='jpeg'&&a!='JPG'&&a!='JPEG'&&a!='PNG'&&a!='png'){
			Pop_up_box("机动车所有人身份证明(正面)图片只能上传.jpg或者.JPEG、png格式的图片！","2");
			return false;
		}
	}
    if($('#fileupload_jbrsfzmfm').val()==''){
		...
		if(a!='jpg'&&a!='jpeg'&&a!='JPG'&&a!='JPEG'&&a!='png'&&a!='PNG'){
			Pop_up_box("机动车所有人身份证明(反面)图片只能上传.jpg或者.JPEG、png格式的图片！","2");
			return false;
		}
	}
	$("#zc_form").submit();
